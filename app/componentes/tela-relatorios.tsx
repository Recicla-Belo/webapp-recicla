...
export function TelaRelatorios({ administrador = false }: { administrador?: boolean }) {
  ...
  async function excluirRegistro(dadosConfirmacao: { senhaAtual: string; confirmacao: string; motivo: string }) {
    ...
    const caminho = alvoExclusao.tipo === "dia"
      ...
      : alvoExclusao.tipo === "pesagem"
        ? "/api/administrador/pesagens"
        : `/api/administrador/auditoria/${alvoExclusao.item.uuid}`;
    setExcluindo(true); setErroExclusao("");
    try {
      await requisitarApi(caminho, { method: "DELETE", body: JSON.stringify(alvoExclusao.tipo === "pesagem"
        ? { ...dadosConfirmacao, confirmacao: "EXCLUIR REGISTROS", pesagensUuids: [alvoExclusao.item.uuid] }
        : dadosConfirmacao) });
  ...
  const descricaoExclusao = alvoExclusao?.tipo === "dia"
    ...
    : alvoExclusao?.tipo === "pesagem"
      ? `A pesagem ${alvoExclusao.item.codigo} será invalidada e seus valores, caixa e metas serão recalculados.`
  ...
  const itensExclusao = alvoExclusao?.tipo === "dia"
    ...
    : alvoExclusao?.tipo === "pesagem"
      ? [`Pesagem ${alvoExclusao.item.codigo}`, `${alvoExclusao.item.catador} · ${alvoExclusao.item.material} · ${numero(alvoExclusao.item.peso_total)} ${alvoExclusao.item.unidade}`, "Cobrança e movimentação operacional vinculadas"]
