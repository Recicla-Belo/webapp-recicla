...
export function TelaCatadores({ podeGerenciar = true }: { podeGerenciar?: boolean }) {
  ...
  const formularioEdicao = podeGerenciar && catadorEdicao && <CadastroCatador cooperativas={cooperativas} edicao={catadorEdicao} onFechar={() => setCatadorEdicao(null)} onSalvo={async () => { await carregar(); if (perfilUuid) setPerfilUuid(null); }} />;
  const modalExclusao = podeGerenciar && catadorExclusao && <ModalExclusaoCatador catador={catadorExclusao} motivo={motivoExclusao} erro={erro} processando={processandoExclusao} aoMudarMotivo={setMotivoExclusao} aoFechar={() => setCatadorExclusao(null)} aoConfirmar={() => void excluirCatador()} />;
  if (perfilUuid) return <><PerfilCatador uuid={perfilUuid} podeGerenciar={podeGerenciar} onVoltar={() => { setPerfilUuid(null); void carregar(); }} onEditar={() => void abrirEdicao(perfilUuid)} onExcluir={abrirExclusao} />{formularioEdicao}{modalExclusao}</>;
  ...
  return <section className={`pagina-interna${podeGerenciar ? "" : " somente-cadastro"}`}>
    ...
    {!podeGerenciar && <p className="aviso-permissao"><LockKeyhole /> Sua conta pode consultar e cadastrar catadores, mas não pode editar, inativar, excluir ou movimentar caixas.</p>}
...
function PerfilCatador({ uuid, podeGerenciar, onVoltar, onEditar, onExcluir }: { uuid: string; podeGerenciar: boolean; onVoltar: () => void; onEditar: () => void; onExcluir: (catador: CatadorApi) => void }) {
  ...
  return <section className="pagina-interna ficha-catador">
    <div className="barra-ficha"><button className="botao-secundario" onClick={onVoltar}><ArrowLeft /> Voltar aos catadores</button><div className="acoes-ficha-catador"><div className="acoes-caixa"><span className={`status-caixa ${caixaHoje?.status ?? "aberto"}`}>Caixa de hoje {caixaHoje?.status ?? "aberto"}</span>{podeGerenciar && (caixaHoje?.status === "fechado" ? <button className="botao-primario" disabled={processando} onClick={() => { setMotivoReabertura(""); setAcaoCaixa("reabrir"); }}><UnlockKeyhole /> Reabrir caixa</button> : <button className="botao-primario" disabled={processando} onClick={() => setAcaoCaixa("fechar")}><LockKeyhole /> Fechar caixa</button>)}</div>{podeGerenciar && <><button className="botao-secundario" type="button" onClick={onEditar}><Pencil /> Editar cadastro</button><button className="botao-perigo" type="button" onClick={() => onExcluir(catador)}><Trash2 /> Excluir catador</button></>}</div></div>
    {erro && <p className="mensagem-erro" role="alert">{erro}</p>}
    ...
    <ModalConfirmacao aberto={Boolean(acaoCaixa)} titulo={acaoCaixa === "reabrir" ? "Reabrir o caixa de hoje?" : "Fechar o caixa de hoje?"} descricao={acaoCaixa === "reabrir" ? "A reabertura será registrada na auditoria e permitirá novas movimentações." : "Depois de fechado, o caixa bloqueará novas pesagens até uma reabertura justificada."} textoConfirmar={acaoCaixa === "reabrir" ? "Reabrir caixa" : "Fechar caixa"} processando={processando} rotuloCampo={acaoCaixa === "reabrir" ? "Motivo obrigatório da reabertura" : undefined} valorCampo={motivoReabertura} aoMudarCampo={setMotivoReabertura} placeholderCampo="Descreva por que o caixa precisa ser reaberto" campoObrigatorio={acaoCaixa === "reabrir"} aoFechar={() => setAcaoCaixa(null)} aoConfirmar={() => acaoCaixa && void alterarCaixa(acaoCaixa)} />
  </section>;
}
