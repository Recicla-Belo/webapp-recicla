...
function PerfilCatador({ uuid, onVoltar, onEditar, onExcluir }: { uuid: string; onVoltar: () => void; onEditar: () => void; onExcluir: (catador: CatadorApi) => void }) {
  ...
  return <section className="pagina-interna ficha-catador">
    ...
    <section className="painel"><div className="titulo-secao"><div><h2>Histórico completo de pesagens</h2><p>Inclui registros cancelados, alterados e excluídos</p></div></div><div className="tabela-responsiva"><table><thead><tr><th>Registro</th><th>Data</th><th>Material</th><th>Operação</th><th>Peso</th><th>Ganho</th><th>Status</th></tr></thead><tbody>{historicoPaginado.map((item) => <tr key={item.uuid}><td><code>{item.codigo}</code></td><td>{new Date(item.data_hora).toLocaleString("pt-BR")}</td><td>{item.material}</td><td>{item.cooperativa || "—"} · {item.ponto_apoio} · {item.responsavel}</td><td>{Number(item.peso_total).toLocaleString("pt-BR")} kg</td><td>{Number(item.valor_total).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</td><td><span className={`status-pesagem ${item.excluida_em ? "excluida" : item.status}`}>{item.excluida_em ? "Excluída" : item.status}</span></td></tr>)}</tbody></table></div><Paginacao pagina={paginaHistorico} total={dados.historico.length} itensPorPagina={itensPerfil} aoMudarPagina={setPaginaHistorico} rotulo="pesagens" /></section>
