...
export function TelaPesagem() {
  ...
  const metaDiaria = Number(metaAtual?.meta ?? material?.meta_diaria ?? 0);
  ...
  const valorBrutoAcumulado = material ? pesoDiaDepois / Number(material.quantidade_referencia) * Number(material.valor_referencia) : 0;
  const ganhoDiaDepois = status === "concluida" && (semMeta || pesoDiaDepois >= metaDiaria) ? valorBrutoAcumulado : Number(metaAtual?.ganho ?? 0);
  const valor = status === "concluida" ? Math.max(ganhoDiaDepois - Number(metaAtual?.ganho ?? 0), 0) : 0;
...
function ProgressoMeta({ percentual, peso, meta, falta, ganho, caixa }: { percentual: number; peso: number; meta: number; falta: number; ganho: number; caixa: CaixaDia | null }) {
  ...
  return <section className="painel-meta"><header><div><span>{semMeta ? "PAGAMENTO IMEDIATO" : "META DIÁRIA"}</span><strong>{semMeta ? `${peso.toLocaleString("pt-BR")} kg registrados` : `${peso.toLocaleString("pt-BR")} de ${meta.toLocaleString("pt-BR")} kg`}</strong></div><b>{semMeta ? "Sem meta" : `${Math.round(percentual)}%`}</b></header><div className="trilho-meta"><i style={{ width: `${percentual}%` }} /></div><footer><span>{semMeta ? "Todo o peso concluído é contabilizado" : falta > 0 ? `Faltam ${falta.toLocaleString("pt-BR")} kg · valor ainda não liberado` : "Meta atingida — os próximos quilos continuam sendo pagos"}</span><strong>{dinheiro(ganho)} no dia</strong></footer>{caixa?.status === "fechado" && <p>Caixa fechado: novas movimentações estão bloqueadas.</p>}</section>;
}
...
function ResumoPesagem({ catador, cooperativa, ponto, material, peso, valor, dataHora, status, percentual, falta, ganhoDia }: { catador?: CatadorApi; cooperativa?: CooperativaApi; ponto?: Referencia; material?: MaterialApi; peso: number; valor: number; dataHora: string; status: StatusPesagem; percentual: number; falta: number; ganhoDia: number }) {
  const semMeta = Number(material?.meta_diaria ?? 0) <= 0;
  return <div className="resumo-pesagem"><div className="linha-resumo"><span>Código do catador e nome</span><strong>{catador?.codigo} — {catador?.nome_completo}</strong></div><div className="linha-resumo"><span>Cooperativa / associação</span><strong>{cooperativa?.nome}</strong></div><div className="linha-resumo"><span>Central / ponto</span><strong>{ponto?.nome}</strong></div><div className="linha-resumo"><span>Material</span><strong>{material?.nome}</strong></div><div className="linha-resumo"><span>Peso</span><strong>{peso.toLocaleString("pt-BR")} {material?.unidade}</strong></div><div className="linha-resumo"><span>Data e hora</span><strong>{dataHora ? new Date(dataHora).toLocaleString("pt-BR") : "—"}</strong></div><div className="linha-resumo"><span>Status</span><strong className={`status-pesagem ${status}`}>{rotulosStatus[status]}</strong></div><div className="linha-resumo"><span>Meta após registro</span><strong>{semMeta ? "Sem meta · pagamento imediato" : `${Math.round(percentual)}% · ${falta > 0 ? `faltam ${falta.toLocaleString("pt-BR")} kg` : "atingida"}`}</strong></div><div className="linha-resumo"><span>Ganho acumulado no dia/material</span><strong>{dinheiro(ganhoDia)}</strong></div><div className="total-pesagem"><span>Valor liberado nesta pesagem</span><strong>{dinheiro(valor)}</strong></div></div>;
}
