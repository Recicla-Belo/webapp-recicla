...
type RespostaPesagem = { codigo: string; valorTotal: number; metaAtingidaAgora: boolean; progressoMeta: { peso: number; ganho: number; metaDiaria: number; percentual: number; falta: number; atingida: boolean } | null; progressoMetaGeral: MetaGeralApi | null };
...
export function TelaPesagem() {
  ...
  const pesoDiaDepois = Number(usaMetaGeral ? metaGeral?.peso : metaAtual?.peso ?? 0) + (status === "concluida" ? Math.max(pesoNumero, 0) : 0);
  ...
  const valorBrutoAcumulado = usaMetaGeral ? Number(metaGeral?.valorBruto ?? 0) + (status === "concluida" ? valorBrutoNovaPesagem : 0) : material ? pesoDiaDepois / Number(material.quantidade_referencia) * Number(material.valor_referencia) : 0;
  ...
  const valor = status === "concluida" ? Math.max(ganhoDiaDepois - ganhoAtual, 0) : 0;
  const metaAtingidaDepois = semMeta || pesoDiaDepois >= metaDiaria;
  const detalhesMetaGeralDepois = atualizarDetalhesMetaGeral(metaGeral?.detalhes ?? [], material, status === "concluida" ? Math.max(pesoNumero, 0) : 0, status === "concluida" ? valorBrutoNovaPesagem : 0, metaAtingidaDepois);
...
function ResumoPesagem({ catador, cooperativa, ponto, material, peso, valor, dataHora, status, percentual, falta, ganhoDia, usaMetaGeral, metaAtingida, detalhes }: { catador?: CatadorApi; cooperativa?: CooperativaApi; ponto?: Referencia; material?: MaterialApi; peso: number; valor: number; dataHora: string; status: StatusPesagem; percentual: number; falta: number; ganhoDia: number; usaMetaGeral: boolean; metaAtingida: boolean; detalhes: DetalheMetaGeralApi[] }) {
  const semMeta = !usaMetaGeral && Number(material?.meta_diaria ?? 0) <= 0;
  const pagamentoVisivel = status === "concluida" && (semMeta || metaAtingida);
  return <div className="resumo-pesagem"><div className="linha-resumo"><span>Código do catador e nome</span><strong>{catador?.codigo} — {catador?.nome_completo}</strong></div><div className="linha-resumo"><span>Cooperativa / associação</span><strong>{cooperativa?.nome}</strong></div><div className="linha-resumo"><span>Central / ponto</span><strong>{ponto?.nome}</strong></div><div className="linha-resumo"><span>Material</span><strong>{material?.nome}</strong></div><div className="linha-resumo"><span>Peso</span><strong>{peso.toLocaleString("pt-BR")} {material?.unidade}</strong></div><div className="linha-resumo"><span>Data e hora</span><strong>{dataHora ? new Date(dataHora).toLocaleString("pt-BR") : "—"}</strong></div><div className="linha-resumo"><span>Status</span><strong className={`status-pesagem ${status}`}>{rotulosStatus[status]}</strong></div><div className="linha-resumo"><span>{usaMetaGeral ? "Meta geral após registro" : "Meta após registro"}</span><strong>{semMeta ? "Sem meta · pagamento imediato" : `${Math.round(percentual)}% · ${falta > 0 ? `faltam ${falta.toLocaleString("pt-BR")} kg` : "atingida"}`}</strong></div>{pagamentoVisivel ? <><div className="linha-resumo"><span>Total liberado no dia</span><strong>{dinheiro(ganhoDia)}</strong></div>{usaMetaGeral && <DetalhesPagamento detalhes={detalhes} />}<div className="total-pesagem"><span>Valor liberado nesta pesagem</span><strong>{dinheiro(valor)}</strong></div></> : <div className="valor-sujeito-meta"><strong>Valores sujeitos ao atingimento da meta</strong><span>Nenhum valor será exibido ou contabilizado antes do alvo.</span></div>}</div>;
}
