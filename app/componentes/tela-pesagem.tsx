...
function CartaoCatador({ catador, caixa, metas, metaGeral }: { catador: CatadorApi; caixa: CaixaDia | null; metas: ProgressoMetaApi[]; metaGeral: MetaGeralApi | null }) {
  ...
  return <div className="confirmar-catador">{catador.tem_foto ? <img className="foto-catador" src={`${URL_API}/api/catadores/${catador.uuid}/foto`} alt={`Foto de ${catador.nome_completo}`} /> : <div className="foto-catador">{catador.nome_completo.split(/\s+/).slice(0, 2).map((p) => p[0]).join("")}</div>}<div><span className="status ativo">● Cadastro ativo</span><h3>{catador.nome_completo}</h3><p><code>{catador.codigo}</code> · {catador.cooperativa ?? "Sem cooperativa"}</p><p>{catador.contatos.map((item) => item.valor).join(" · ") || "Contato não informado"}</p><p>{catador.endereco_resumo || "Endereço não informado"}</p>{metaGeral?.ativa ? <div className="mini-meta"><span>Meta geral do ciclo: {metaGeral.peso.toLocaleString("pt-BR")} / {metaGeral.meta.toLocaleString("pt-BR")} {metaGeral.unidade}</span><b>{Math.round(metaGeral.percentual)}%</b></div> : melhorMeta && <div className="mini-meta"><span>Melhor meta do ciclo: {melhorMeta.nome}</span><b>{Math.round(melhorMeta.percentual)}%</b></div>}</div><div className="selo-confirmacao"><span><Check /></span><small>Identidade conferida</small><strong className={`status-caixa ${caixa?.status === "fechado" ? "fechado" : "aberto"}`}>Ciclo {caixa?.status ?? "aberto"}</strong></div></div>;
}
...
function PagamentoForaMeta({ peso, unidade, valor, caixa }: { peso: number; unidade: string; valor: number; caixa: CaixaDia | null }) {
  return <section className="painel-meta painel-fora-meta"><header><div><span>FORA DA META</span><strong>{peso.toLocaleString("pt-BR")} {unidade} com pagamento imediato</strong></div><b>{dinheiro(valor)}</b></header><footer><span>Este peso não altera a meta do ciclo do catador.</span><strong>Preço configurado preservado na pesagem</strong></footer>{caixa?.status === "fechado" && <p>Ciclo fechado: novas movimentações estão bloqueadas.</p>}</section>;
}
