...
import { requisitarApi, type MaterialApi } from "@/app/dados/api";
...
export function TelaConfiguracoes() {
  ...
  const [formMaterial, setFormMaterial] = useState({ nome: "", tipoMaterial: "Outro", unidade: "kg", quantidadeReferencia: "1", valorReferencia: "0", ativo: true });
  ...
  useEffect(() => { void carregarMateriais(); }, [carregarMateriais]);

  function abrirMaterial(material?: MaterialApi) {
    setMaterialEdicao(material ?? null);
    setFormMaterial(material ? { nome: material.nome, tipoMaterial: material.tipo_material, unidade: material.unidade, quantidadeReferencia: String(material.quantidade_referencia), valorReferencia: String(material.valor_referencia), ativo: material.status === "ativo" } : { nome: "", tipoMaterial: "Outro", unidade: "kg", quantidadeReferencia: "1", valorReferencia: "0", ativo: true });
    setModal(true);
  }

  async function salvarMaterial() {
    try {
      await requisitarApi(materialEdicao ? `/api/materiais/${materialEdicao.uuid}` : "/api/materiais", { method: materialEdicao ? "PUT" : "POST", body: JSON.stringify({ ...formMaterial, quantidadeReferencia: Number(formMaterial.quantidadeReferencia.replace(",", ".")), valorReferencia: Number(formMaterial.valorReferencia.replace(",", ".")) }) });
  ...
  return <section className="pagina-interna configuracoes">
    ...
    {aba === "identidade" ? <div className="painel-identidade">
      ...
      {mensagem && <p className="mensagem-configuracao" role="status">{mensagem}</p>}
      <div className="lista-materiais">{materiais.map((material) => <article key={material.uuid}><span className="amostra-material">{material.tipo_material.slice(0, 2).toUpperCase()}</span><div className="nome-material"><strong>{material.nome}</strong><small>{material.tipo_material} · {material.unidade.toUpperCase()}</small></div><div><small>Referência de pagamento</small><strong>{Number(material.valor_referencia).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })} a cada {Number(material.quantidade_referencia)} {material.unidade}</strong></div><span className={material.status === "ativo" ? "status ativo" : "status"}>● {material.status}</span><button className="botao-secundario" onClick={() => abrirMaterial(material)}><Pencil /> Editar</button><button className="menu-acoes perigoso" onClick={() => void excluirMaterial(material)} aria-label={`Excluir ${material.nome}`}><Trash2 /></button></article>)}</div>
