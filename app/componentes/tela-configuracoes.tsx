...
import { Ban, CheckCircle2, DatabaseZap, KeyRound, MapPinned, Pencil, Plus, ShieldCheck, Target, Trash2, UserRound, X } from "lucide-react";
import { requisitarApi, type AdministradorApi, type ConfiguracaoMetaGeralApi, type MaterialApi, type PontoApoioApi, type ResponsavelPesagemApi } from "@/app/dados/api";
...
type AbaConfiguracao = "materiais" | "responsaveis" | "pontos" | "identidade" | "usuarios" | "conta" | "limpeza";
...
export function TelaConfiguracoes({ administrador, onAdministradorAtualizado }: { administrador: AdministradorApi; onAdministradorAtualizado: (administrador: AdministradorApi) => void }) {
  ...
  const podePontos = administrador.administrador || administrador.permissoes.includes("pontos_apoio_gerenciar");
  ...
  const abaInicial: AbaConfiguracao = podeMateriais || podeMetas ? "materiais" : podeResponsaveis ? "responsaveis" : podePontos ? "pontos" : "identidade";
  ...
  const [pontoEdicao, setPontoEdicao] = useState<PontoApoioApi | null>(null);
  const [formPonto, setFormPonto] = useState({ nome: "", ativo: true });
  ...
  const [metaGeral, setMetaGeral] = useState({ ativa: false, metaDiaria: "0", valorPremio: "0", unidade: "kg" });
  const [salvandoMetaGeral, setSalvandoMetaGeral] = useState(false);
  const [confirmacaoExclusao, setConfirmacaoExclusao] = useState<{ tipo: "material" | "responsavel" | "ponto"; item: MaterialApi | ResponsavelPesagemApi | PontoApoioApi } | null>(null);
  const [paginaMateriais, setPaginaMateriais] = useState(1);
  const [paginaResponsaveis, setPaginaResponsaveis] = useState(1);
  const [paginaPontos, setPaginaPontos] = useState(1);
  ...
  const carregarPontos = useCallback(async () => { try { const dados = await requisitarApi<{ dados: PontoApoioApi[] }>("/api/pontos-apoio?incluirInativos=true"); setPontos(dados.dados); setPaginaPontos((paginaAtual) => Math.min(paginaAtual, Math.max(1, Math.ceil(dados.dados.length / itensPorPagina)))); } catch (erro) { setMensagem(erro instanceof Error ? erro.message : "Não foi possível carregar os pontos de apoio."); } }, []);
  ...
  useEffect(() => {
    ...
    if (podePontos) carregamentos.push(carregarPontos());
    if (podeMetas) carregamentos.push(carregarMetaGeral());
    void Promise.all(carregamentos);
  }, [carregarMateriais, carregarMetaGeral, carregarPontos, carregarResponsaveis, podeMateriais, podeMetas, podePontos, podeResponsaveis]);
  ...
  function abrirPonto(ponto?: PontoApoioApi) {
    if (!podePontos) return;
    setPontoEdicao(ponto ?? null);
    setFormPonto(ponto ? { nome: ponto.nome, ativo: ponto.status === "ativo" } : { nome: "", ativo: true });
    setMensagem(""); setModalPonto(true);
  }

  async function salvarPonto() {
    if (!podePontos) return;
    try {
      await requisitarApi(pontoEdicao ? `/api/pontos-apoio/${pontoEdicao.uuid}` : "/api/pontos-apoio", { method: pontoEdicao ? "PUT" : "POST", body: JSON.stringify(formPonto) });
      setModalPonto(false); await carregarPontos(); setMensagem("Ponto de apoio salvo com sucesso.");
    } catch (erro) { setMensagem(erro instanceof Error ? erro.message : "Não foi possível salvar o ponto de apoio."); }
  }

  async function excluirPonto(ponto: PontoApoioApi) {
    if (!podePontos) return;
    try { await requisitarApi(`/api/pontos-apoio/${ponto.uuid}`, { method: "DELETE" }); await carregarPontos(); setMensagem("Ponto de apoio excluído com sucesso."); }
    catch (erro) { setMensagem(erro instanceof Error ? erro.message : "Não foi possível excluir o ponto de apoio."); }
  }
  ...
  async function confirmarExclusao() {
    ...
    else if (tipo === "responsavel") await excluirResponsavel(item as ResponsavelPesagemApi);
    else await excluirPonto(item as PontoApoioApi);
  }
  ...
  async function salvarMetaGeral() {
    ...
    if (!Number.isFinite(premio) || premio < 0 || (metaGeral.ativa && premio <= 0)) { setMensagem("Informe um prêmio fixo maior que zero para ativar a meta geral."); return; }
    setSalvandoMetaGeral(true); setMensagem("");
    try {
      await requisitarApi("/api/configuracoes/meta-geral", { method: "PUT", body: JSON.stringify({ ativa: metaGeral.ativa, metaDiaria: valor, valorPremio: premio, unidade: metaGeral.unidade }) });
      setMensagem("Meta geral atualizada. O prêmio foi recalculado em todos os ciclos abertos; caixas fechados permanecem imutáveis.");
  ...
  const pontosPaginados = pontos.slice((paginaPontos - 1) * itensPorPagina, paginaPontos * itensPorPagina);
  ...
  return <section className={`pagina-interna configuracoes${podeMateriais ? "" : " sem-gerenciar-materiais"}${podeMetas ? "" : " sem-gerenciar-meta"}`}>
    <div className="abas-configuracao" role="tablist" aria-label="Configurações">
      ...
      {podePontos && <button className={aba === "pontos" ? "ativo" : ""} onClick={() => setAba("pontos")} role="tab" aria-selected={aba === "pontos"}>Pontos de apoio</button>}
    ...
    {aba === "limpeza" ? <section className="painel painel-limpeza-dados">
      ...
      <div className="indicadores-zerados"><strong>Após a limpeza, serão recalculados como zero:</strong><p>Total coletado · Valor total a pagar · Média por catador · Coletas realizadas · Catadores que bateram a meta no ciclo atual.</p></div>
      ...
      <div className="resumo-pagina"><div><h2>Pontos de apoio</h2><p>Gerencie os locais disponíveis para novas pesagens. Pontos inativos permanecem preservados no histórico.</p></div>{podePontos && <button className="botao-primario" onClick={() => abrirPonto()}><Plus /> Novo ponto de apoio</button>}</div>
      {mensagem && <p className="mensagem-configuracao" role="status">{mensagem}</p>}
      <div className="lista-responsaveis">{pontos.length === 0 ? <p className="estado-vazio">Nenhum ponto de apoio cadastrado.</p> : pontosPaginados.map((ponto) => <article key={ponto.uuid}><span><MapPinned /></span><div><strong>{ponto.nome}</strong><small>{ponto.status === "ativo" ? "Disponível para novas pesagens" : "Ponto de apoio inativo"}</small></div><em className={ponto.status === "ativo" ? "status ativo" : "status"}>● {ponto.status}</em><button className="botao-secundario" onClick={() => abrirPonto(ponto)}><Pencil /> Editar</button><button className="menu-acoes perigoso" onClick={() => setConfirmacaoExclusao({ tipo: "ponto", item: ponto })} aria-label={`Excluir ${ponto.nome}`}><Trash2 /></button></article>)}</div>
      <Paginacao pagina={paginaPontos} total={pontos.length} itensPorPagina={itensPorPagina} aoMudarPagina={setPaginaPontos} rotulo="pontos de apoio" />
    ...
    {modalPonto && <div className="sobreposicao" role="dialog" aria-modal="true" aria-labelledby="titulo-ponto"><div className="modal pequeno"><header className="cabecalho-modal"><div><span>LOCAL DA OPERAÇÃO</span><h2 id="titulo-ponto">{pontoEdicao ? "Editar ponto de apoio" : "Novo ponto de apoio"}</h2><p>Somente pontos ativos aparecem no formulário de nova pesagem.</p></div><button onClick={() => setModalPonto(false)} aria-label="Fechar">×</button></header><form className="formulario" onSubmit={(e) => e.preventDefault()}><label className="campo">Nome do ponto de apoio<input value={formPonto.nome} maxLength={160} onChange={(e) => setFormPonto((atual) => ({ ...atual, nome: e.target.value }))} /></label><label className="interruptor compacto"><input type="checkbox" checked={formPonto.ativo} onChange={(e) => setFormPonto((atual) => ({ ...atual, ativo: e.target.checked }))} /><span /><div><strong>Disponível para novas pesagens</strong><small>{formPonto.ativo ? "Ponto ativo" : "Ponto desativado"}</small></div></label></form><footer className="rodape-modal"><button className="botao-secundario" onClick={() => setModalPonto(false)}>Cancelar</button><button className="botao-primario" onClick={() => void salvarPonto()} disabled={formPonto.nome.trim().length < 2}>Salvar ponto de apoio</button></footer></div></div>}
    ...
    <ModalConfirmacao aberto={Boolean(confirmacaoExclusao)} titulo={`Excluir ${confirmacaoExclusao?.item.nome ?? "registro"}?`} descricao={confirmacaoExclusao?.tipo === "material" ? "A exclusão só será permitida quando o material não possuir pesagens vinculadas." : confirmacaoExclusao?.tipo === "ponto" ? "A exclusão definitiva só será permitida se o ponto não possuir pesagens. Se houver histórico, desative-o pela opção Editar." : "O cadastro será excluído definitivamente. Nas pesagens antigas, somente o nome do responsável continuará preservado para auditoria."} textoConfirmar="Confirmar exclusão" perigoso aoFechar={() => setConfirmacaoExclusao(null)} aoConfirmar={() => void confirmarExclusao()} />
  </section>;
}
